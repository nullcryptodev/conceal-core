...
std::string Ipv4Address::toDottedDecimal() const {
  std::string result;
  result += std::to_string(m_value >> 24);
  result += '.';
  result += std::to_string(m_value >> 16 & 255);
  result += '.';
  result += std::to_string(m_value >> 8 & 255);
  result += '.';
  result += std::to_string(m_value & 255);
  return result;
}

bool Ipv4Address::isLoopback() const {
  // 127.0.0.0/8
  return (m_value & 0xff000000) == (127 << 24);
}

bool Ipv4Address::isPrivate() const {
  return
    // 10.0.0.0/8
    (m_value & 0xff000000) == (10 << 24) ||
    // 172.16.0.0/12
    (m_value & 0xfff00000) == ((172 << 24) | (16 << 16)) ||
    // 192.168.0.0/16
    (m_value & 0xffff0000) == ((192 << 24) | (168 << 16));
}
